...
router.post("/:cid/producto/:pid", passport.authenticate('jwt', {session: false}), async (req, res) => {
    try {
...
router.put("/:cid/producto/:pid", passport.authenticate('jwt', {session: false}), async (req, res) => {
    try {
        let { cid, pid } = req.params
        let { cantidad } = req.body
